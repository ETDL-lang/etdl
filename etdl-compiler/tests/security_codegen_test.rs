//! Compile-check + run-check for the Security Supplement's codegen (Part
//! A.5): registration, `security.control_effective` branch selection, and
//! the live-aware `record_branch` probability argument. Mirrors
//! `safety_codegen_test.rs` exactly — the same harness, a different ECEL
//! path and a different check (a single declared ceiling, not a band).
//!
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../etdl-cli/tests/fixtures/security-check.etdl")
}
...
#[test]
fn security_fixture_compiles_and_runs_with_live_branch_selection() {
    ...
    assert!(generated.contains("<= 0.020000"));
    assert!(generated.contains("etdl_ensure_live_gateway_bypass_registered"));
    ...
    // `gencheck/src/main.rs`'s `gen-check-security` block drives
    ...
    // live estimate drifts above the control's declared bypass ceiling —
    // a failed assertion there panics, so a non-zero exit here means the
    // live behavior didn't actually happen.
    let status = Command::new("cargo")
        .args(["run", "--quiet", "--features", "gen-check-inline,gen-check-security"])
    ...
    assert!(
        status.success(),
        "security-check fixture's generated code failed at runtime (see gencheck's own \
