...
`etdl.diagnostics`, `etdl.security`, and `etdl.reliability` when the
...
etdl.performance: available (etdl.performance/1.0) — Declared latency-percentile budgets (p50Ms/p95Ms/p99Ms) and an optional throughput expectation for an Operation or a whole Event Tree; declarative only, never enforced at runtime.
...
### etdl supplement install|list|remove
...
extensions (compiled in via Cargo feature flags). Compiled in only with the
`plugins` feature (**not** a default feature — it pulls in `wasmtime`, a full
Cranelift JIT). See [Supplement Plugins](reference/supplement-plugins.md) for
...
document that doesn't declare it. When the `plugins` feature is not compiled
in, these subcommands report a clear "support is not enabled in this build"
message and exit non-zero, the same convention `etdl reliability` follows
when the `reliability` feature is off.
